...
import {X,Filter,People,Check,CalendarWeek,CurrencyDollar, Search, XCircle,Download,CheckCircle} from "react-bootstrap-icons";
...
const TableTopBar = ({rows,onRowsFilterChange, InvoicesData, selectedRowCount,selectClass,selectedRow }) => {
  ...
  useEffect(() => {
    if (InvoicesData) {
      const calculatedTotalAmount = InvoicesData.reduce(
  ...
  const fullNames = [...new Set(InvoicesData.map(item => item.client.name))];
  const PaymentStatus = [...new Set(InvoicesData.map(item => item.paid))];


  
  const fullclientJson = JSON.stringify(fullNames, null, 2);
  const fullclientArray = JSON.parse(fullclientJson);
  const PaymentStatusJson = JSON.stringify(PaymentStatus, null, 2);
  const PaymentStatusArray = JSON.parse(PaymentStatusJson);

  const getStatusCount = (status) => {
    return InvoicesData.filter(item => item.paid === status).length;
  };
  ...
  const [key, setKey] = useState(fullclientJson);
  ...
  const groupSelectedItems = () => {
    ...
    selectedItems.forEach((item) => {
      if (fullclientArray.includes(item)) {
        groupedItems['Client'].push(item);
      } else if (selectedRange.includes(item)) {
        groupedItems['DateRange'].push(item);
      }else if (PaymentStatusArray.includes(item)) {
        groupedItems.Payment.push(item);
      }
    });
    return groupedItems;
  };
...
const filteredclient = fullclientArray.filter((itemName) =>
  ...
  <Col style={{ textAlign: "right" }}>
    {InvoicesData && InvoicesData.length > 0 ? (
      <p className="flexEndStyle styleT3" >
        Total <span className="styleT2">{InvoicesData.length} Invoices</span>{" "}
        
      </p>
    ) : (
      <p className="flexEndStyle styleT3">
        Total <span className="styleT2">0 Invoices</span>
      </p>
    )}
  </Col>
</Row>
     ...
     {buttonClicked && (
      <Tabs
       ...
       className="filtterBoxWrapper">
        ...
        <Tab eventKey="Client" title={<><People color="#667085" size={16} /> Client</>}>
          <ul>
        ...
        <Search color='#98A2B3' size={20} />
            ...
            {filteredclient.map((itemName, index) => (
        ...
        <Tab eventKey="Payment" title={<><CurrencyDollar color="#667085" size={16} />Payment</>}>
          <ul>
            {PaymentStatusArray.map((itemName, index) => (
              <li key={index} className={selectedItems.includes(itemName) ? 'checkedList' : ''}>
