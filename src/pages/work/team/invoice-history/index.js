            ...
            <div className='client-order-history' style={{ width: visible ? 'calc(100% - 559px)' : '100%' }}>
                <div className={`topbar ${selected?.length ? style.active : ''}`} style={{ padding: '4px 32px 4px 23px', position: 'relative', height: '48px' }}>
                    <div className='left-side d-flex align-items-center' style={{ gap: '16px' }}>
                        {
                            selected?.length ? (
                                ...
                                <>
                                    <Link to={"/work/people"} className={`${style.gobackBtn}`}><ChevronLeft color='#1AB2FF' size={20} /> Go Back</Link>
                    ...
                    <div className="featureName d-flex align-items-center" style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}>
                        <h1 onClick={() => { setVisible(true); }} className={`${style.clientName} m-0 p-0 cursor-pointer`} title={name}>{name || ""}</h1>
                    </div>
                    <div className="right-side d-flex align-items-center" style={{ gap: '8px' }}>
                        <div className={`${style.status} ${style.paid}`}>Paid: 0</div>
                        <div className={`${style.status} ${style.unpaid}`}>Not Paid: 0</div>
                        <Button label={
                            <>
                ...
                <TeamInvoiceHistoryTable ref={dt} selected={selected} setSelected={setSelected} searchValue={debouncedValue} setVisible={setVisible} />
            </div>
...
export default TeamInvoiceHistory;