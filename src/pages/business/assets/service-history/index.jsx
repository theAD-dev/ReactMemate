...
import { ChevronLeft, Download, PlusCircle } from 'react-bootstrap-icons';
...
const ServiceHistory = () => {
    ...
    return (
        <>
            <Helmet>
                <title>Service History - {vehicleQuery?.data?.registration_number || 'Vehicle'}</title>
            </Helmet>

            <div className='service-history-page'>
                <div className='service-history' style={{ width: '100%' }}>
                    <div className={`topbar ${selected?.length ? style.active : ''}`} style={{ padding: '4px 32px 4px 23px', position: 'relative', height: '48px' }}>
                        ...
                        <div className="featureName d-flex align-items-center" style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}>
                            <h1 className={clsx(style.vehicleName, 'm-0 p-0')} title={vehicleQuery?.data?.registration_number}>
                                {vehicleQuery?.data?.registration_number || "Loading..."}
                            </h1>
                        </div>
